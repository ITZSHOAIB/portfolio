...
{#if icon}
  <svg class={`${className} shrink-0`} viewBox="0 0 24 24" role="img" aria-label={label}>
    <path fill={brandColor ? iconFill(icon.hex) : 'currentColor'} d={icon.path}></path>
  </svg>
{:else}
  <span
    ...
    style={fallbackColor ? `background: ${fallbackColor}; border-color: ${fallbackColor}` : undefined}
    aria-label={label}
  >
